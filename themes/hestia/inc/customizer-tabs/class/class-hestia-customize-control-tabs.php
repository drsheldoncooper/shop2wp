...
class Hestia_Customize_Control_Tabs extends WP_Customize_Control {
	...
	public function render_content() {
		...
		foreach ( $this->tabs as $value => $args ) {
			if ( ! empty( $args['controls'] ) && ( $this->tab_has_controls( $args['controls'] ) ) ) {
				...
				$output .= '</label>';
				$output .= '</div><!-- /.hestia-customizer-tab -->';
			}
		}
		$output .= '</div><!-- /.hestia-tabs-control -->';
	...
	public function enqueue() {
		...
		wp_enqueue_script( 'hestia-tabs-control-script', get_template_directory_uri() . '/inc/customizer-tabs/js/script.js', array( 'jquery' ), HESTIA_VERSION, true );
		wp_enqueue_style( 'hestia-tabs-control-style', get_template_directory_uri() . '/inc/customizer-tabs/css/style.css', null, HESTIA_VERSION );
	...
	public function partials_helper_script_enqueue() {
		wp_enqueue_script( 'hestia-tabs-addon-script', get_template_directory_uri() . '/inc/customizer-tabs/js/customizer-addon-script.js', array( 'jquery' ), HESTIA_VERSION, true );
	}
